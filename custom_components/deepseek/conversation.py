...
class DeepSeekConversationEntity(
    ...
    async def async_process(
        ...
        base_url = options.get(CONF_BASE_URL, config.get(CONF_BASE_URL, DEFAULT_BASE_URL))
        ...
        show_reasoning = options.get(CONF_REASONING, config.get(CONF_REASONING, DEFAULT_REASONING))
...
Be concise, helpful, and friendly. If you're not sure about something, say so."""
    ...
    async def async_reload(self, hass: HomeAssistant, config: conversation.ConversationAgentConfig) -> None:
        """Reload the agent."""
        # Nothing to reload
        pass
    
    async def async_get_debug_info(self, hass: HomeAssistant, config: conversation.ConversationAgentConfig) -> dict:
